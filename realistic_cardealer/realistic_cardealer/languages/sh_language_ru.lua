RCD = RCD or {}
RCD.Language = RCD.Language or {}

RCD.Language["ru"] = {
	["undefined"] = "неизвестно",
	["adminMenuConfig"] = "АДМИН МЕНЮ - НАСТРОЙКА",
	["configureGroupsVehicles"] = "Настроить группы и автомобили",
	["groups"] = "ГРУППЫ",
	["vehicles"] = "АВТОМОБИЛИ",
	["createGroup"] = "СОЗДАТЬ НОВУЮ ГРУППУ",
	["createVehicle"] = "ДОБАВИТЬ НОВЫЙ АВТОМОБИЛЬ",
	["dealerConfiguration"] = "НАСТРОЙКА АВТОДИЛЕРА",
	["enterDealerName"] = "Введите имя автодилера",
	["groupsConfig"] = "Настройка групп",
	["newPlateforms"] = "НОВЫЕ ПЛАТФОРМЫ ДЛЯ СОЗДАНИЯ",
	["deletePlateforms"] = "УДАЛИТЬ ВСЕ ПЛАТФОРМЫ",
	["deleteNPC"] = "УДАЛИТЬ NPC",
	["saveInformations"] = "СОХРАНИТЬ ВСЮ ИНФОРМАЦИЮ",
	["configureGroups"] = "Настроить информацию о группах",
	["rankAccess"] = "ДОСТУП ПРИВИЛЕГИЯМ",
	["jobAccess"] = "ДОСТУП РАБОТАМ",
	["enterGroupName"] = "Введите название группы",
	["cancel"] = "ОТМЕНИТЬ",
	["validateCreateGroup"] = "ПРОВЕРКА И СОЗДАНИЕ ГРУППЫ",
	["configureVehicleInformations"] = "Настройка информации о машине",
	["preview"] = "ПРЕДПОКАЗ",
	["chooseVehicleClass"] = "Выберите класс автомобиля...",
	["chooseVehicleGroup"] = "Выберите группу автомобиля...",
	["enterVehicleName"] = "Введите название автомобиля",
	["enterVehiclePrice"] = "Введите цену автомобиля",
	["vehicleView"] = "ПОКАЗ АВТОМОБИЛЯ",
	["generalSettings"] = "ОСНОВНЫЕ НАСТРОЙКИ",
	["priceSettings"] = "НАСТРОЙКИ ЦЕН",
	["validateCreateVehicle"] = "ПРОВЕРКА И СОЗДАНИЕ АВТОМОБИЛЯ",
	["configureAddon"] = "Настроить всю информацию дополнения",
	["beltConfig"] = "НАСТРОЙКА РЕМНЯ БЕЗОПАСТНОСТИ",
	["engineConfig"] = "НАСТРОЙКА ДВИГАТЕЛЯ",
	["speedometerConfig"] = "НАСТРОЙКА СПИДОМЕТРА",
	["driveConfig"] = "НАСТРОЙКА ДИСОК",
	["configureCarDealers"] = "НАСТРОЙКА АВТОДИЛЕРОВ",
	["validateSaveSettings"] = "ПРОВЕРКА И СОХРАНЕНИЕ НАСТРОЕК",
	["underglow"] = "Нижняя подсветка",
	["skins"] = "Облики",
	["bodygroups"] = "Группы тела",
	["carDealer"] = "Автодилер",
	["spawnVehicle"] = "СОЗДАТЬ АВТОМОБИЛЬ",
	["bringBack"] = "ВЕРНУТЬ",
	["testVehicle"] = "ТЕСТ АВТОМОБИЛЯ",
	["customize"] = "НАСТРОЙКА (%s)",
	["sell"] = "ПРОДАТЬ",
	["buyVehicle"] = "Приобрести",
	["customizationTitle"] = "ПЕРСОНАЛИЗАЦИЯ",
	["mainMenuTitle"] = "ГЛАВНОЕ МЕНЮ",
	["modifyVehicle"] = "МОДИФИЦИРОВАНИЕ ВАШЕГО АВТОМОБИЛЯ",
	["buySaleVehicles"] = "ПРИОБРЕСТИ ИЛИ ПРОДАТЬ АВТОМОБИЛИ",
	["vehicleOwned"] = "ПРИОБРЕТЁННЫЕ",
	["vehicleForSale"] = "ДЛЯ ПОКУПКИ",
	["allowed"] = "ДОСТУПНЫЕ",
	["cantCustomizeVehicle"] = "Вы не можете персонализировать это авто. \n приносим извинения за причинённые неудобства",
	["testDrive"] = "Тест-драйв",
	["testDriveEnd"] = "Тест-драйв завершится через %s сек.",
	["addonNotInstalled"] = "Дополнение %s не установлено",
	["vehicleTooFar"] = "Ваш автомобиль находится слишком далеко, чтобы настроить его",
	["customizeVehicleText"] = "Вы персонализировали своё авто за %s",
	["invalidUsergroup"] = "У вас нет подходящего ранга, чтобы сделать это",
	["invalidJob"] = "У вас неподходящая работа для этого",
	["cantAfford"] = "У вас недостаточно денег, чтобы сделать это",
	["buyVehicleText"] = "Вы приобрели %s за %s",
	["cantReturnVehicle1"] = "Вы не можете продать свой автомобиль, когда он сломан",
	["sellVehicleText"] = "Вы продали свой автомобиль %s за %s",
	["testVehicleText"] = "Вы приступили к тестированию автомобиля, у вас есть %s сек.",
	["vehicleAlreadyExited"] = "Этот автомобиль уже создан",
	["maxVehicleLimitReached"] = "Вы достигли максимального количества созданных авто",
	["cantSpawnVehicle2"] = "Вы можете забрать авто во время теста",
	["noPlaceAvailable"] = "Свободных мест нет, приходите позже",
	["vehicleExitedText2"] = "Вы создали авто %s",
	["returnAroundVehicles"] = "Вы только что убрали авто вокруг вас",
	["noVehiclesAround"] = "Вокруг вас нет никакого автомобиля",
	["vehicleTooFarText"] = "Ваш автомобиль находится слишком далеко, чтобы его можно было вернуть",
	["vehicleReturned2"] = "Вы только что вернули свой автомобиль",
	["npcTooFar"] = "Вы находитесь слишком далеко от NPC, чтобы выполнить это действие",
	["npcNotOwnedVehicle"] = "У NPC нет этого транспортного средства",
	["cantExitVehicle2"] = "Вы не можете выйти из машины, потому что она не стоит на месте",
	["cantExitVehicle3"] = "Вы не можете выйти из машины, потому что пристёгнуты ремнём безопасности",
	["invalidGroupName"] = "Название группы недействительно",
	["vehicleGroupEdited"] = "Вы изменили группу %s",
	["vehicleGroupCreated"] = "Вы создали группу автомобилей %s",
	["invalidGroupVehicle"] = "Группа автомобилей недействительна",
	["vehicleGroupDeleted"] = "Вы удалили группу автомобилей #%s",
	["invalidVehicleClass"] = "Класс автомобиля недействителен",
	["invalidVehicleName"] = "Название автомобиля неверен",
	["invalidVehiclePrice"] = "Цена автомобиля недействительна",
	["vehicleEdited"] = "Вы модифицировали автомобиль %s",
	["vehicleCreated"] = "Вы создали автомобиль %s",
	["vehicleDeleted"] = "Вы удалили автомобиль #%s",
	["pnjUpdated"] = "Вы обновили NPC %s",
	["pnjDeleted"] = "Вы удалили NPC №%s",
	["plateformEditMode"] = "Сейчас вы находитесь в режиме редактирования, размещая платформы для создания",
	["beenSaved"] = "Вы сохранили %s платформ",
	["allPlateformsDeleted"] = "Вы удалили все платформы на своём сервере",
	["serverConfigurationUpdated"] = "Вы обновили настройки своего сервера",
	["close"] = "Закрыть",
	["car"] = "Автомобиль",
	["returnButton"] = "Вернуться",
	["maxSpeed"] = "Максимальная скорость",
	["horsePower"] = "Лошадиных сил",
	["wheelsPerAxles"] = "Колёса на ось",
	["posX"] = "Позиция по гориз.",
	["angleX"] = "Поворот по гориз.",
	["posY"] = "Позиция по вертик.",
	["AngleY"] = "Поворот по вертик.",
	["posZ"] = "Позиция по диагон.",
	["angleZ"] = "Поворот по диагон.",
	["fov"] = "Угол обзора",
	["useCustomNotification"] = "Используйте настраиваемое уведомление",
	["enterIntoVehicle"] = "Войти в авто",
	["currency"] = "Валюта",
	["adminCommand"] = "Админ команда",
	["maxVehicle"] = "Максимум авто",
	["testTime"] = "Время тест-драйва",
	["distanceToReturn"] = "Дистанция для возврата",
	["underglowKey"] = "Кнопка подсвечения",
	["activateBelt"] = "Пристегнуть ремень безопасности",
	["warningSound"] = "Звук гудка",
	["beltKey"] = "Кнопка ремня безопасности",
	["activateEngine"] = "Завести двигатель",
	["engineKey"] = "Кнопка двигателя",
	["timeToLunchVehicle"] = "Время запуска автомобиля",
	["cantLeaveVehicleInMotion"] = "Нельзя оставлять автомобиль в движении",
	["activateSmallAccident"] = "Включить небольшую аварию",
	["activateEjectionAccident"] = "Включить вылет из машины",
	["lowerSpeedToExit"] = "Более низкая скорость для выхода",
	["minDamageSmallAccident"] = "Минимальный урон при небольшой аварии",
	["minDamageEjectionAccident"] = "Минимальный ущерб при вылете из машины",
	["activateSpeedometer"] = "Включить спидометр",
	["activateSimfphysSpeedometer"] = "Включить спидометр из simfphys для авто дополнения simfphys",
	["speedometerSize"] = "Размер спидометра",
	["count"] = "Счёт",
	["space"] = "Расстояние",
	["defaultSkin"] = "Стандартный облик",
	["choose"] = "Выбор",
	["skin"] = "Облик",
	["defaultColor"] = "Стандартный цвет",
	["chooseColor"] = "Выбор цвета",
	["configureDefaultColor"] = "Настроить стандартный цвет",
	["setDefaultColor"] = "УСТАНОВИТЬ СТАНДАРТНЫЙ ЦВЕТ",
	["canModifyBodygroup"] = "Может изменить группы тела",
	["canModifySkin"] = "Может изменять облик",
	["canModifyColor"] = "Может изменять цвет",
	["canModifyUngerglow"] = "Может изменять подсветку",
	["canTestVehicle"] = "Может тестировать авто",
	["canBuyNitro"] = "Может приобрести нитро",
	["priceToChangeBodygroups"] = "Цена за смену группы тела",
	["priceToChangeSkins"] = "Цена за смену облика",
	["priceToChangeColors"] = "Цена за смену цветов",
	["colors"] = "Цвета",
	["nitro"] = "Нитро",
	["priceToChangeUnderglow"] = "Цена за смену подсветки",
	["priceToBuyNitro"] = "Цена покупки нитро",
	["pourcentSell"] = "Процент от продажи",
	["noText"] = "Нет текста",
	["invalidText"] = "Текст проблемы",
	["language"] = "Язык",
	["speedUnit"] = "Единица измерения скорости",
	["free"] = "БЕСПЛАТНО",
	["buyForFree"] = "ПРИОБРЕСТИ БЕСПЛАТНО",
	["customizePrice"] = "У вас недостаточно денег для персонализации этого автомобиля",
	["notEnoughtMoney"] = "У тебя недостаточно денег, чтобы сделать это",
	["rentVehc"] = "Вы арендовали автомобиль %s за %s",
	["cantSwitch"] = "Вы не можете пересесть, потому что пристёгнуты ремнём безопасности",
	["canSellWithJob"] = "Вы можете арендовать автомобиль, когда работаете автодилером",
	["brakePower"] = "Тормозная мощность",
	["modifyRentVehicle"] = "Вы изменили информацию о транспортном средстве",
	["refundRentVehicle"] = "Вам были возвращены деньги за %s",
	["turnSpeed"] = "Скорость поворота",
	["mass"] = "Масса",
	["owned"] = "ПРИОБРЕТЕНО",
	["cardealerJobSettings"] = "НАСТРОИТЬ РАБОТУ АВТОДИЛЕРА",
	["cardealerJobDiscount"] = "Процентная цена автомобиля при продаже игроком",
	["minCommissionPrice"] = "Минимальная комиссионная цена",
	["maxCommissionPrice"] = "Максимальная комиссионная цена",
	["alreadyBought"] = "Вы не можете купить этот автомобиль, потому что он у вас уже есть",
	["notGoodTeam"] = "У тебя нет хорошей работы, чтобы сделать это",
	["rentPrice"] = "Арендная плата за работу автодилера",
	["vcmodTransfert"] = "Перенести все данные из VCMOD",
	["wcdTransfert"] = "Перенести все данные из WCD",
	["advancedTransfert"] = "Перенести все данные из ACD",
	["modernTransfert"] = "Перенести все данные из MODERN",
	["compatibilities"] = "ПАРАМЕТРЫ СОВМЕСТИМОСТИ",
	["transfertStart"] = "Вы начали передачу данных, это может занять немного времени...",
	["alreadyTransfert"] = "Вы уже передали эти данные",
	["problem"] = "Возникла проблема, пожалуйста, повторите попытку",
	["invoiceOptions"] = "Настройка(-и)",
	["invoiceCarName"] = "Название автомобиля",
	["invoiceOptionPrice"] = "Цена настройки",
	["invoiceTotalPrice"] = "Итоговая цена",
	["sellerSignature"] = "Подпись продавца",
	["buyerSignature"] = "Подпись покупателя",
	["invoice"] = "Счёт-фактура",
	["seller"] = "Продавец: %s",
	["chooseSkin"] = "Выберите облик",
	["chooseVehicle"] = "Выберите автомобиль",
	["sign"] = "Подписать",
	["print"] = "Отпечаток",
	["openAdminConfiguration"] = "ОТКРЫТЬ АДМИН НАСТРОЙКИ",
	["selectVehicle"] = "Вам нужно выбрать автомобиль",
	["noVehicles"] = "У вас нет никакого автомобиля для настройки",
	["toolName"] = "Установить NPC",
	["toolDesc"] = "Поставить NPC и настроить его",
	["toolLeft1"] = "Нажмите ЛКМ, чтобы создать платформу",
	["toolLeft2"] = "Нажмите ЛКМ, чтобы создать или изменить NPC",
	["toolRight1"] = "Нажмите ПКМ, чтобы удалить последнюю созданную платформу",
	["toolRight2"] = "ПКМ по NPC, чтобы удалить его",
	["toolReload1"] = "Нажмите ПЕРЕЗАРЯДКУ, чтобы сохранить платформы",
	["toolReload2"] = "Нажмите ПЕРЕЗАРЯДКУ, чтобы перезапустить всех NPC",
	["invoiceLimit"] = "Вы достигли лимита вашего счёта",
	["stand"] = "ПОЗИЦИЯ",
	["pressE"] = "НАЖМИТЕ E",
	["yourCommission"] = "Ваши комиссионные",
	["rentVehicle"] = "Арендуйте автомобиль за %s",
	["saveRentInformation"] = "СОХРАНИТЬ ИНФОРМАЦИЮ",
	["startEngineProblem"] = "При запуске вашего автомобиля возникает проблема с двигателем",
	["giveVehicle"] = "У вас есть автомобиль %s для SteamID %s",
	["carDealerJobConfig"] = "Название работы автодилера",
	["areYouSure"] = "ВЫ УВЕРЕНЫ?",
	["reset"] = "СБРОСИТЬ",
	["acceptedInvoice"] = "%s приобретает автомобиль %s за %s",
	["cantAcceptYourInvoice"] = "Вы не можете принять своё предложение",
	["import"] = "Импорт",
	["standTitle"] = "АВТОДИЛЕР",
	["sellVehicle"] = "Продать автомобиль",
	["rentVehicleSpace"] = "У вас нет места, чтобы арендовать автомобиль",
	["nitroKey"] = "Кнопка нитро",
	["nitroSpeed"] = "Ускоритель скорости с нитро",
	["minSpeedNitro"] = "Минимальная скорость для активации нитро",
	["cantSell"] = "Вы не можете продать этот автомобиль",
	["cantSellSetting"] = "Не могу продать этот автомобиль",
	["playersManagement"] = "Управление игроками",
	["open"] = "Открыть",
	["searchDisconnected"] = "ОТКЛЮЧЁННЫЙ ИГРОК",
	["connectedPlayers"] = "ПОДКЛЮЧЁННЫЕ ИГРОКИ",
	["selectedPlayer"] = "ВЫБРАННЫЙ ИГРОК",
	["allVehicles"] = "ВСЕ АВТОМОБИЛИ",
	["adminMenuPlayer"] = "АДМИНИСТРАТОР - ИГРОКИ",
	["adminMenuPlayerDesc"] = "Управление автомобилями, купленными вашими игроками",
	["modifiedInformation"] = "Вы изменили информацию о %s",
	["searchSteamId2"] = "Поиск по SteamID / SteamID64",
	["refreshList"] = "ОБНОВИТЬ СПИСОК",
	["searchPlayer"] = "ПОИСК ИГРОКА",
	["savePlayerInfo"] = "СОХРАНИТЬ ИНФОРМАЦИЮ О ИГРОКЕ",
	["nitroConfig"] = "НАСТРОЙКА НИТРО",
	["nitroDuration"] = "Продолжительность нитро",
	["nitroCooldowns"] = "Время перезарядки нитро",
	["cooldownsNitroNotify"] = "Ваш нитро не готов! Следующее использование через %s сек.",
	["alreadyRented"] = "Вы уже арендовали этот автомобиль",
	["sellVehicleRented"] = "Вы получили %s за продажу автомобиля %s",
	["boatSettingText"] = "Является ли транспортное средство лодкой?",
	["disableBeltVehicle"] = "Отключить модуль ремней безопасности для данного автомобиля",
	["disableEngineVehicle"] = "Отключить модуль двигателя для данного автомобиля",
	["defaultUnderglow"] = "Подсветка по умолчанию",
	["defaultBodygroups"] = "Группы тел по умолчанию",
	["chooseBodygroups"] = "Выберите группы тел",
	["chooseUnderglow"] = "Выберите подсветку",
	["configuredefaultBodygroups"] = "Настройка групп тел по умолчанию",
	["configuredefaultUnderglow"] = "Настройка подсветки по умолчанию",
	["setDefaultBodygroups"] = "Установить группы тел по умолчанию",
	["setDefaultUnderglow"] = "Установить подсветку по умолчанию",
	["usePrecacheModels"] = "Предварительно кэшировать модели для оптимизации игры игроков",
	["serverName"] = "Имя вашего сервера",
	["cantTest"] = "Вы не можете испытать автомобиль",
	["notPurchasable"] = "НЕ ПОКУПАЕТСЯ",
	["cantBuy"] = "Не может купить автомобиль",
	["maxInvoice"] = "Максимальное количество счётов-фактур, которые можно создать одновременно",
	["maxVehicles"] = "Максимальные транспортные средства",
	["configureMaxVehicles"] = "Настройка максимального количества транспортных средств",
	["saveMaxVehicles"] = "ПОДТВЕРДИТЬ ПАРАМЕТРЫ",
	["noLoadHealth"] = "Не сохранять жизнь транспортных средств",
	["lockVehicle"] = "Запирайте автомобиль при нересте",
	["resellerTitle"] = "РЕСЕЛЛЕР АВТО",
	["resellerConfigTitle"] = "НАСТРОЙКА РЕСЕЛЛЕРА",
	["resellerDesc"] = "Продавайте украденные автомобили и получайте свои деньги",
	["resellerSell"] = "ПРОДАТЬ АВТОМОБИЛЬ ЗА %s",
	["insurancePourcentPrice"] = "Страховая ставка, основанная на цене автомобиля",
	["insuranceSettings"] = "НАСТРОЙКА СТРАХОВКИ",
	["insuranceModuleActivated"] = "Используйте систему страхования",
	["removeVehicleInsurance"] = "Постоянно удаляйте уничтоженные автомобили, если игрок не имеет страховки",
	["removeVehicleEverytime"] = "Постоянно удаляйте автомобили, уничтоженные в любом состоянии",
	["payInsurance"] = "СТРАХОВКА (%s)",
	["insuredVehicle"] = "ЗАСТРАХОВАННЫЙ АВТОМОБИЛЬ",
	["vehicleTooFarAction"] = "Ваш автомобиль слишком далеко, чтобы сделать это действие",
	["insurancePrice"] = "У вас недостаточно денег, чтобы заплатить за страховку на этот автомобиль",
	["resellerDesc"] = "Настройте имя и модель реселлера",
	["resellPourcentPrice"] = "Продажная ставка, основанная на цене автомобиля",
	["maxResellPrice"] = "Максимальная цена перепродажи автомобилей",
	["maxInsurancePrice"] = "Максимальная цена страховки автомобилей",
	["vehicleSpeedSell"] = "Автомобиль должен быть остановлен, чтобы продать",
	["soldStolenVehicle"] = "Вы только что продали автомобиль за %s",
	["cantSellOwnVehicle"] = "Вы не можете продать свой автомобиль",
	["vehicleStolenRemove"] = "Ваш автомобиль был украден и будет окончательно потерян! В следующий раз купите страховку",
	["vehicleStolenNotRemove"] = "Ваш автомобиль был украден",
	["cantResellVehicle"] = "Отключить перепродажу автомобилей у реселлера",
	["noVehicleAround"] = "Рядом нет автомобилей",
	["pourcentPriceSellVehicle"] = "Продажная ставка, основанная на цене автомобиля (0 берёт стандартную настройку ставки)",
	["pourcentInsuranceVehicle"] = "Страховая ставка, основанная на цене автомобиля (0 берёт стандартную настройку ставки)",
	["noPermission"] = "У вас нет разрешения на это",
	["garageDisabled"] = "Вы не можете выполнить это действие, потому что автодилер не является гаражом",
	["shopDisabled"] = "Вы не можете выполнить это действие, потому что автодилер не является магазином",
	["disableGarage"] = "Отключить гараж",
	["disableShop"] = "Отключить магазин",
	["timeWithInsurance"] = "Время ожидания, когда автомобиль украден с страховкой",
	["timeWithoutInsurance"] = "Время ожидания, когда автомобиль украден без страховки",
	["timeToWaitNotify"] = "Ваш автомобиль был украден, вам нужно подождать %s, прежде чем он снова станет доступным",
	["someoneInVehicle"] = "В автомобиле есть кто-то",
	["distanceToSell"] = "Максимальное расстояние для продажи автомобиля",
	["timeToStoleNewVehicle"] = "Время, через которое можно украсть новый автомобиль",
	["notifyToStoleNewVehicle"] = "Вам нужно подождать %s, прежде чем украсть новый автомобиль",
	["vehicleDestroyed"] = "Вы не можете выполнить это действие, так как автомобиль уничтожен",
	["cantReturnVehicleDestroyed"] = "Нельзя вернуть автомобиль, когда он уничтожен",
	["removeVehicleStolen"] = "Постоянное удаление украденных автомобилей, если игрок не имеет страховки",
	["noLoadFuel"] = "Не сохранять топливо автомобилей",
	["scale"] = "Масштаб модели",
	["searchVehicle"] = "Поиск автомобиля",
	["searchBarActivate"] = "Включите строку поиска для поиска автомобилей",
	["disableLockVehicle"] = "Отключить автоматическую блокировку этого автомобиля",
	["garageNoShop"] = "При отключении гаража на нпс вы не увидите покупаемые автомобили",
	["shopNoGarage"] = "При отключении магазина на npc вы не увидите принадлежащие ему автомобили",
	["carDealerNotConfigured"] = "На автодилере нет доступных транспортных средств",
	["defaultNitro"] = "Нитро по умолчанию",
	["none"] = "Нет",
	["descAddValue"] = "Добавить значение в список",
	["addValue"] = "ДОБАВИТЬ ЗНАЧЕНИЕ",
	["insertValue"] = "Вставить значение",
	["vehiclesWithoutEngine"] = "Автомобили без двигателя",
	["vehiclesWithoutBelt"] = "Автомобили без ремня безопасности",
}
